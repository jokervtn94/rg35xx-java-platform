...
- Font runtime deployment: 011N requires an explicit absolute `RG35XX_FONT_RUNTIME_PATH`; disposable build paths must never be embedded in `fonts.properties`. `scripts/rc1_runtime_build_overlay.sh` emits `rg35xx_runtime_files.list` with staged source, target path and SHA-256 for later packaging/deployment.
- First consolidated build toolchain carrier: `docker.io/miyoocfw/toolchain-shared-uclibc@sha256:6f6761867b4e4dcc27c99bf25fb91b2910264165f27bdd40b1c17e6f98cf751e`. 011U/011V verified `/opt/miyoo/bin/arm-miyoo-linux-uclibcgnueabi-{gcc,g++,nm,readelf}`, exact target triple `arm-miyoo-linux-uclibcgnueabi`, GCC/G++ `(Buildroot -g8087b523) 9.4.0`, and an ARMv5TE EABI5 soft-float probe. Workflow evidence: run `33862107194`, artifact `9932521250`, probe SHA-256 `80d5e4a2e0f824a535a03ac4910d61db43ff3e9a4bbf02edc5dfd08c5e6af138`.
- Historical/fallback toolchain source provenance: `MiyooCFW/buildroot@8087b52311da5c1e2fa1c50b0b064c07fd174a36`, configuration `configs/miyoo_uclibc_defconfig`, Git blob `58256d3e7afa3f9fa0dbd79b16fd0b21d93b9d7b`. 011S remains the exact source-build fallback. Project run `33843201574` timed out while rebuilding the complete SDK and produced no accepted SDK artifact; it is not the default first-build path.
- The old MiyooCFW toolchain v2.0.0 binary release remains rejected because it is musl, not uClibc.
- First build compiler contract: 011O requires explicit ARM/uClibc GCC/G++ executables and verifies their target triplets. Target flags are `-marm -march=armv5te -mtune=arm926ej-s -mfloat-abi=soft`; target `nm` must report no unresolved `rg35xx_*` symbols in the resulting core.

## Closed static gates

010K media process boundary; 010L direct MIDI/WAV facade; 010M ToneControl; 010N vendor-container capability boundary; 010O native media consolidation; 011B graphics/input/lifecycle; 011C pinned RMS baseline; 011D headless font ownership; 011E TML/TSF provenance; 011F consolidated prebuild gate; 011G external runtime/SoundFont provenance; 011H font-resource provider/provenance; 011I native build manifest/core-link ownership; 011J reproducible FreeJ2ME assembly; 011K runtime/Make assembly structure; 011L Classpath/font compiler preflight; 011M concrete headless FontPeer overlay/source reconciliation; 011N font runtime path/deployment ownership; 011O first-build harness/native Make graph integration; 011S source-pinned ARM/uClibc fallback provenance; 011U/011V pinned prebuilt uClibc carrier probe/acceptance.

## Authoritative integration patches / overlays

0003 Manager media profile; 0004 audio pipe; 0005 audio bootstrap; 0006 native PlatformPlayer; 0007 drawRGB; 0008 transform cache; 0009 historical RMS policy (superseded by 0021); 0010 lifecycle; 0011 image cache; 0012 dirty-frame (blocking consumer superseded by 0020); 0013 input; 0014 media events; 0015 native media runtime; 0016 audio FD inheritance; 0017 process-boundary completion; 0018 direct MIDI/WAV; 0019 ToneControl; 0020 pinned graphics/input/lifecycle; 0021 pinned RMS baseline; 0022 GNU Classpath headless FontPeer contract; `runtime/classpath/apply_rg35xx_font_overlay.sh` is the 011M/011N concrete 0.99 realization of 0022; `scripts/rc1_runtime_build_overlay.sh` owns the 011O disposable Makefile.common include; `scripts/rc1_compile.sh` owns the first consolidated Java/ARM compile procedure. `scripts/rc1_toolchain_acquire.sh` and `scripts/rc1_toolchain_materialize.sh` remain the 011S/011T source-build fallback path; 011U/011V select the digest-pinned prebuilt uClibc carrier for the default first consolidated build.

Other integration contracts are applied only by the deterministic 011J assembly against the exact pinned source; divergence is a failure, never permission to duplicate helpers. The Classpath overlay similarly refuses unexpected baseline source shapes and 011N rejects a disposable build path as a runtime font mapping.
...
Consolidation requires every KEEP source exactly once, no superseded RG35XXFontEngine resurrection, exact TML/TSF headers, pinned SoundFont bytes, pinned/materialized DejaVu font with recorded SHA-256, concretely overlaid GNU Classpath headless peer source, explicit runtime font deployment path/manifest, consolidated native call-sites and the single native link manifest before BUILD-PASS. 011O additionally requires the single generated Make overlay to be included exactly once and the target compiler/nm checks to pass. 011U/011V additionally require the immutable prebuilt uClibc image digest and target probe evidence to remain exact; 011S/011T remain the fallback if that carrier is unavailable. STATIC source closure still does not replace Classpath/JamVM compilation or target ARMv5TE/uClibc compile/link validation.
