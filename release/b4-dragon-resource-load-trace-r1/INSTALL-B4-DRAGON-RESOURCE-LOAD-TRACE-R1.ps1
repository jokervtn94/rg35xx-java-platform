...
$ExpectedOldRuntime='fa953382169425b087418a09a6d496c142a86f5e2e5c75af760d0b8d263a41f0'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-RESOURCE-LOAD-TRACE-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-GAMECANVAS-FLUSH-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-RESOURCE-LOAD-TRACE-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-resource-load-trace-r1-$stamp"
...
try {
 ...
 @(
  ...
  'RESOURCE_LOAD_TRACE_DIAGNOSTICS=ENABLED',
  'IMAGE_DECODE_TRACE_DIAGNOSTICS=ENABLED',
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-RESOURCE-LOAD-TRACE-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-RESOURCE-LOAD-TRACE-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
 Write-Host 'INSTALL PASS - B4 DRAGON GAMECANVAS FLUSH R1'
}
catch {
