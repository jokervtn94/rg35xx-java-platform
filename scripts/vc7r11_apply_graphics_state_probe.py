...
# The verified-clean transport logs requestFrame across three source lines.
# Match the exact pinned source and fail closed if it drifts.
old = '''        System.err.println("RG35XX-JAVA-DIAG: requestFrame ENTER " + sourceWidth + "x" + sourceHeight +\n                           " data=" + (sourceData == null ? -1 : sourceData.length) +\n                           " lock=" + (sourceLock == null ? "null" : sourceLock.getClass().getName()));'''
new = '''        System.err.println("RG35XX-JAVA-DIAG: requestFrame ENTER " + sourceWidth + "x" + sourceHeight +\n                           " data=" + (sourceData == null ? -1 : sourceData.length) +\n                           " lock=" + (sourceLock == null ? "null" : sourceLock.getClass().getName()) +\n                           " RG35XX-VC7R11-FRAME-BIND dataId=" + System.identityHashCode(sourceData) +\n                           " lockId=" + System.identityHashCode(sourceLock));'''
if u.count(old) != 1:
    raise SystemExit('VC7R11 FAIL requestFrame diagnostic anchor count=%d' % u.count(old))
if u.count('public void requestFrame(int sourceWidth, int sourceHeight,') != 1:
    raise SystemExit('VC7R11 FAIL requestFrame signature count=%d' % u.count('public void requestFrame(int sourceWidth, int sourceHeight,'))
