...
function Fail([string]$m){throw "R2C FONT RESTORE FAIL: $m"}
...
if(!(Test-Path -LiteralPath $SdRoot -PathType Container)){Fail "SD root not found"}
$Sd=(Resolve-Path -LiteralPath $SdRoot).Path
$ptr=Join-Path $Sd 'RG35XX-R2C-FONT-CURRENT-BACKUP.txt'
...
Write-Host "RESTORE R2C FONT PASS from $backup"
