...
function Fail([string]$m){throw "RG35XX R2B AUDIO RESTORE FAIL: $m"}
...
$Sd=(Resolve-Path -LiteralPath $SdRoot).Path;$ptr=Join-Path $Sd 'RG35XX-CLEAN-R2B-AUDIO-CURRENT-BACKUP.txt';if(!(Test-Path -LiteralPath $ptr -PathType Leaf)){Fail 'backup pointer missing'};$backup=(Get-Content -LiteralPath $ptr -Raw).Trim();$state=Join-Path $backup 'STATE.txt';if(!(Test-Path -LiteralPath $state -PathType Leaf)){Fail "STATE missing: $state"}
foreach($line in @(Get-Content -LiteralPath $state)){$p=$line.Split('|');if($p.Count -lt 2){continue};$kind=$p[0];$rel=$p[1];$dst=Join-Path $Sd $rel;$bak=Join-Path $backup $rel;if(Test-Path -LiteralPath $dst){Remove-Item -LiteralPath $dst -Force};if($kind -eq 'EXISTED' -and (Test-Path -LiteralPath $bak -PathType Leaf)){Ensure-Parent $dst;Copy-Item -LiteralPath $bak -Destination $dst -Force}}
Write-Host "RESTORE R2B AUDIO PASS from $backup"
