...
# Exact upstream + exact historical media/audio source.
...
# ARMv5TE/uClibc toolchain used by the device-proven core line.
...
# Restore the historical Java media layer and native audio backend, but NOT the
# VC7R4 RGB reference-strip diagnostic. This is the first clean-screen native
# build after the color diagnostic series.
...
# Graphics/runtime fixes. Deliberately keep transform cache disabled and do not
# apply vc7r4_apply_video_color_diag.py or vc7r5_apply_java_frame_probe.py.
...
# Clean-screen gate: RGB test/reference strip must not exist in source.
...
strings "$CORE" | grep -Fq 'RG35XX-VC7R3-AUDIO-NATIVE'
