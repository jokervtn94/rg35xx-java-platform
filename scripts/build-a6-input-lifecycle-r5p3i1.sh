...
[ -z "$(git -C "$UPSTREAM" status --porcelain --untracked-files=no)" ] || fail "canonical dirty after input lifecycle staging"
...
[ "$FIX_SHA" != "$PARENT_SHA" ] || fail "input lifecycle candidate identical to R5P3"

# Exact class-entry set must remain unchanged.
...
# Only RG35XXLauncher.class may differ byte-for-byte from R5P3.
python3 - "$PARENT_JAR" "$FIX_JAR" <<'PY'
import sys, zipfile, hashlib
base, new = sys.argv[1], sys.argv[2]
with zipfile.ZipFile(base) as a, zipfile.ZipFile(new) as b:
    na=set(a.namelist()); nb=set(b.namelist())
    if na != nb: raise SystemExit('A6_R5P3I1_SCOPE_FAIL entry-set')
    diff=[]
    for n in sorted(na):
        if hashlib.sha256(a.read(n)).digest() != hashlib.sha256(b.read(n)).digest():
            diff.append(n)
allowed=['org/recompile/rg35xx/RG35XXLauncher.class']
if diff != allowed:
...
need=['RG35XX_A6_TRACE_INPUT_START_BEFORE_RUNJAR','RG35XX_A6_TRACE_RUNJAR_BEGIN','RG35XX_A6_TRACE_RUNJAR_RETURN']
for x in need:
    if x not in s: raise SystemExit('A6_R5P3I1_ORDER_GATE_FAIL marker='+x)
pos_start=s.find('RG35XXLauncher$InputPump.start')
pos_run=s.find('MobilePlatform.runJar')
if pos_start < 0 or pos_run < 0 or pos_start >= pos_run:
    raise SystemExit('A6_R5P3I1_ORDER_GATE_FAIL start=%d run=%d'%(pos_start,pos_run))
print('A6_R5P3I1_INPUT_BEFORE_RUNJAR_GATE=PASS')
PY

grep -q 'RG35XX_A6_TRACE_KEYPRESS_ENTER' "$BUILD/a6-r5p3i1-mobileplatform.javap" || fail "R5 keypress trace lost"
grep -q 'RG35XXCore2D.sourceOver' "$BUILD/a6-r5p3i1-platformgraphics.javap" || fail "R5P3 polygon fix lost"

R5P3I1OUT="$ROOT/out/a6-realgame-r5p3i1"
rm -rf "$R5P3I1OUT"; mkdir -p "$R5P3I1OUT"
cp "$FIX_JAR" "$R5P3I1OUT/freej2me-rg35xx.jar"
cp "$ROOT/out/a6-realgame-r5p3/JAVA6-COMPAT-AUDIT.tsv" "$R5P3I1OUT/JAVA6-COMPAT-AUDIT.tsv"
cp "$ROOT/out/a6-realgame-r5p3/CANONICAL-DIFF-MANIFEST.txt" "$R5P3I1OUT/CANONICAL-DIFF-MANIFEST.txt"
cat >> "$R5P3I1OUT/CANONICAL-DIFF-MANIFEST.txt" <<EOF
...
A6_R5P3I1_CHANGED_CLASS=org/recompile/rg35xx/RG35XXLauncher.class
CANONICAL_GITLINK_MUTATED=NO
EOF
cp "$BUILD/a6-r5p3i1-launcher.javap" "$R5P3I1OUT/A6-R5P3I1-LAUNCHER-JAVAP.txt"
cp "$BUILD/a6-r5p3i1-mobileplatform.javap" "$R5P3I1OUT/A6-R5P3I1-MOBILEPLATFORM-JAVAP.txt"
cp "$BUILD/a6-r5p3i1-platformgraphics.javap" "$R5P3I1OUT/A6-R5P3I1-PLATFORMGRAPHICS-JAVAP.txt"
cat > "$R5P3I1OUT/A6-REALGAME-R5P3I1-IDENTITY.txt" <<EOF
...
STABLE=NO
EOF
(cd "$R5P3I1OUT" && sha256sum * > SHA256SUMS.txt)
echo A6_R5P3I1_BUILD=PASS
cat "$R5P3I1OUT/A6-REALGAME-R5P3I1-IDENTITY.txt"
