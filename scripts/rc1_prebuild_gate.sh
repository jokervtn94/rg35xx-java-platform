...
PIN_FONT_SOURCE_COMMIT="0eda8a319c08835009849583cd090bb5b141ce25"
...
[ ! -e "$ROOT/src/org/recompile/mobile/RG35XXFontEngine.java" ] || fail "superseded RG35XXFontEngine.java was reintroduced without a REVERT/REPLACE task"
...
for p in 0020-pinned-graphics-input-lifecycle-consolidation.patch 0021-pinned-rms-safe-baseline.patch 0022-pinned-headless-font-peer-consolidation.patch; do
...
if [ "$MODE" = "--build-ready" ]; then
  ...
  : "${RG35XX_FONT_FILE:?set RG35XX_FONT_FILE to the materialized DejaVu 2.37 TTF resource}"
  : "${RG35XX_FONT_SHA256:?set RG35XX_FONT_SHA256 to the locally recorded SHA-256 for that exact TTF}"
  ...
  for f in src/org/recompile/mobile/PlatformGraphics.java src/org/recompile/mobile/PlatformFont.java src/org/recompile/mobile/PlatformImage.java src/org/recompile/mobile/MobilePlatform.java src/org/recompile/freej2me/Libretro.java src/libretro/freej2me_libretro.c; do
    [ -f "$RG35XX_FREEJ2ME_ROOT/$f" ] || fail "pinned FreeJ2ME source missing: $f"
  done
  ...
  FONT_SHA256=$(sha256sum "$RG35XX_FONT_FILE" | awk '{print $1}')
  [ "$FONT_SHA256" = "$RG35XX_FONT_SHA256" ] || fail "font SHA-256 does not match recorded assembly provenance"
  note "font source pin: DejaVu 2.37 commit $PIN_FONT_SOURCE_COMMIT; binary SHA-256 verified from assembly record"
