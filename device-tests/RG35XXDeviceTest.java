...
public final class RG35XXDeviceTest extends MIDlet {
    ...
    protected void startApp(){ if(canvas==null)canvas=new TestCanvas(this); Display.getDisplay(this).setCurrent(canvas); canvas.start(); }
    ...
    static final class TestCanvas extends Canvas implements Runnable,PlayerListener {
        ...
        private String rmsStatus="PENDING",mediaStatus="PENDING",mediaEvent="none";
        ...
        TestCanvas(RG35XXDeviceTest m){midlet=m;setFullScreenMode(true);prepareGraphics();runRms();}
        void start(){running=true;if(loop==null){loop=new Thread(this,"RG35XXDeviceTestV2");loop.start();}}
        ...
        protected void paint(Graphics g){int w=getWidth(),h=getHeight();g.setColor(0x101820);g.fillRect(0,0,w,h);header(g,w);int y=36;if(page==HOME)home(g,y);else if(page==VIDEO)video(g,y,w,h);else if(page==GRAPHICS)graphics(g,y,w,h);else if(page==FONT)font(g,y);else if(page==INPUT)input(g,y);else if(page==RMS)rms(g,y);else if(page==MEDIA)media(g,y);else summary(g,y);footer(g,w,h);}
        private void header(Graphics g,int w){g.setColor(0x162A3A);g.fillRect(0,0,w,30);g.setColor(0xFFFFFF);g.setFont(Font.getFont(Font.FACE_SYSTEM,Font.STYLE_BOLD,Font.SIZE_MEDIUM));g.drawString("RG35XX RC1 DEVICE TEST v2",8,5,Graphics.TOP|Graphics.LEFT);}
        ...
        private void home(Graphics g,int y){title(g,"1/8 Overview",y);text(g,"Run from normal RG35XX Java menu.",10,y+36);text(g,"No terminal required.",10,y+56);text(g,"RMS boot counter: "+rmsBoot,10,y+88);text(g,"Use LEFT/RIGHT through all 8 pages.",10,y+120);text(g,"SUMMARY shows PASS/PENDING/FAIL diagnostics.",10,y+140);}
        ...
        private void input(Graphics g,int y){title(g,"5/8 Input",y);text(g,"Press buttons; hold one key for repeats.",10,y+38);text(g,"last key="+lastKey+" action="+actionName(lastAction),10,y+76);text(g,"pressed="+pressCount+" released="+releaseCount+" repeats="+repeatCount,10,y+110);text(g,"PASS requires press + release + repeat activity.",10,y+145);}
        private void rms(Graphics g,int y){title(g,"6/8 RMS",y);text(g,"Persistent boot counter="+rmsBoot,10,y+44);text(g,"status="+rmsStatus,10,y+72);text(g,"Exit/reopen JAR: counter must increase.",10,y+110);text(g,"FIRE reruns same-session verify.",10,y+135);}
        private void media(Graphics g,int y){title(g,"7/8 Media",y);String[] n={"Manager.playTone","WAV 8 kHz","MIDI","ToneControl"};text(g,"UP/DOWN choose, FIRE play",10,y+38);for(int i=0;i<n.length;i++){g.setColor(i==mediaMode?0xFFD166:0xE8EEF2);g.drawString((i==mediaMode?"> ":"  ")+n[i],18,y+70+i*25,Graphics.TOP|Graphics.LEFT);}text(g,"status="+mediaStatus,10,y+190);text(g,"event="+mediaEvent,10,y+215);text(g,"WAV/MIDI/ToneControl PASS on END_OF_MEDIA.",10,y+245);}
        private void summary(Graphics g,int y){title(g,"8/8 SUMMARY",y);g.setFont(Font.getDefaultFont());state(g,"VIDEO",tick>20?"PASS":"PENDING",y+42);state(g,"GRAPHICS",(trnsImage!=null&&spriteImage!=null)?"PASS":"FAIL",y+67);Font f=Font.getDefaultFont();state(g,"FONT",(f.getHeight()>0&&f.stringWidth("RG35XX")>0)?"PASS":"FAIL",y+92);state(g,"INPUT",(pressCount>0&&releaseCount>0&&repeatCount>0)?"PASS":"PENDING",y+117);state(g,"RMS",rmsStatus.indexOf("PASS")==0?"PASS":"FAIL",y+142);state(g,"PLAYTONE",playToneCalled?"PASS":"PENDING",y+167);state(g,"WAV",wavPass?"PASS":"PENDING",y+192);state(g,"MIDI",midiPass?"PASS":"PENDING",y+217);state(g,"TONE CONTROL",tonePass?"PASS":"PENDING",y+242);text(g,"Diagnostic only - real-device review still required.",10,y+280);}

        public void keyPressed(int k){lastKey=k;lastAction=safeAction(k);pressCount++;if(k==KEY_NUM0){midlet.exit();return;}if(lastAction==LEFT)page=(page+PAGES-1)%PAGES;else if(lastAction==RIGHT)page=(page+1)%PAGES;else if(page==MEDIA&&lastAction==UP)mediaMode=(mediaMode+3)%4;else if(page==MEDIA&&lastAction==DOWN)mediaMode=(mediaMode+1)%4;else if(lastAction==FIRE){if(page==RMS)runRms();else if(page==MEDIA)runMedia();}repaint();}
        ...
        private int safeAction(int k){try{return getGameAction(k);}catch(Throwable t){return 0;}}
