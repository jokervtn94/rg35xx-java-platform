...
def read(p): return p.read_text().replace('\r\n','\n').replace('\r','\n')
...
if old not in s or 'OP_REGISTER_MIDI_FILE' in s: raise SystemExit('Java protocol anchor mismatch/already patched')
write(jproto,s.replace(old,new,1))
...
if anchor not in s or 'boolean registerMidiFile(' in s: raise SystemExit('Java transport anchor mismatch/already patched')
write(jt,s.replace(anchor,insert,1))

s=read(jp)
if 'private static File cacheMidiToFile(' in s: raise SystemExit('NativePlayer already patched')
...
if old not in s: raise SystemExit('NativePlayer constructor anchor mismatch')
...
if old not in s: raise SystemExit('NativePlayer setMidi anchor mismatch')
s=s.replace(old,new,1)
...
if old not in s: raise SystemExit('NativePlayer prefetch anchor mismatch')
s=s.replace(old,new,1)
old='''        RG35XXMediaRegistry.release(entry.playerId);\n        media = null;\n        state = CLOSED;\n'''
new='''        RG35XXMediaRegistry.release(entry.playerId);\n        media = null;\n        if(midiFile != null)\n        {\n            try { midiFile.delete(); } catch(Exception ignored) { }\n            midiFile = null;\n        }\n        state = CLOSED;\n'''
if old not in s: raise SystemExit('NativePlayer close anchor mismatch')
s=s.replace(old,new,1)
...
if anchor not in s or 'private static File cacheMidiToFile(' in s: raise SystemExit('NativePlayer helper anchor mismatch/already patched')
write(jp,s.replace(anchor,helper,1))

s=read(nproto)
old='    RG35XX_AUDIO_RESET          = 10\n'
new='    RG35XX_AUDIO_RESET          = 10,\n    RG35XX_AUDIO_REGISTER_MIDI_FILE = 11\n'
if old not in s or 'REGISTER_MIDI_FILE' in s: raise SystemExit('native protocol anchor mismatch/already patched')
write(nproto,s.replace(old,new,1))

s=read(ndisp)
if 'static int rg35xx_audio_register_midi_file(' in s: raise SystemExit('native dispatch already patched')
...
if anchor not in s: raise SystemExit('native dispatch helper anchor mismatch')
...
if old not in s: raise SystemExit('native dispatch switch anchor mismatch')
write(ndisp,s.replace(old,new,1))
