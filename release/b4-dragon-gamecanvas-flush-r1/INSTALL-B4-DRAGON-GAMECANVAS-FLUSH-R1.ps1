...
$ExpectedOldRuntime='9929d9ae95105bf296b1cae9cfe997b301ce1016f2813ad6265da21f13f6fd41'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-GAMECANVAS-FLUSH-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-CANONICAL-FRAMEBUFFER-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-GAMECANVAS-FLUSH-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-gamecanvas-flush-r1-$stamp"
...
try {
 ...
 @(
  ...
  'GAMECANVAS_FLUSH_DIAGNOSTICS=ENABLED',
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-GAMECANVAS-FLUSH-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-GAMECANVAS-FLUSH-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
