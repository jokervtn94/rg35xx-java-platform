...
$ExpectedOldRuntime='c6f2c8c71fdb120dac1b8193952d0839dc4cd6877ba8c2e120f606cfb087faab'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-NETWORK-TRACE-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-DISPLAY-TRACE-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-NETWORK-TRACE-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-network-trace-r1-$stamp"
...
try {
 ...
 @(
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-NETWORK-TRACE-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-NETWORK-TRACE-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
