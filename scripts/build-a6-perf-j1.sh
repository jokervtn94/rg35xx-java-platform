...
# Removing a private field from an inner class may rewrite InnerClasses / constant-pool
# metadata across the enclosing class family. Permit binary changes only inside the
# RG35XXLauncher family, then prove unaffected members have identical javap -p -c output.
...
if not diff:
  raise SystemExit('A6_PERF_J1_SCOPE_FAIL candidate identical to parent')
for n in diff:
  if not (n.startswith('org/recompile/rg35xx/RG35XXLauncher') and n.endswith('.class')):
    raise SystemExit('A6_PERF_J1_SCOPE_FAIL changed='+repr(diff))
required='org/recompile/rg35xx/RG35XXLauncher$FramePresenter.class'
if required not in diff:
  raise SystemExit('A6_PERF_J1_SCOPE_FAIL FramePresenter unchanged changed='+repr(diff))
print('A6_PERF_J1_CHANGED_ENTRIES='+','.join(diff))
print('A6_PERF_J1_BINARY_SCOPE_GATE=PASS')
PY
...
# Semantic bytecode lock: outer launcher, anonymous helpers and InputPump must be
# instruction-identical. Only FramePresenter is allowed a behavioral delta.
check_same_class() {
  local cls="$1" tag="$2"
  "$JAVA8/bin/javap" -classpath "$PARENT_JAR" -p -c "$cls" > "$BUILD/a6-perf-j1-parent-$tag.javap"
  "$JAVA8/bin/javap" -classpath "$FIX_JAR" -p -c "$cls" > "$BUILD/a6-perf-j1-candidate-$tag.javap"
  if ! cmp -s "$BUILD/a6-perf-j1-parent-$tag.javap" "$BUILD/a6-perf-j1-candidate-$tag.javap"; then
    diff -u "$BUILD/a6-perf-j1-parent-$tag.javap" "$BUILD/a6-perf-j1-candidate-$tag.javap" || true
    fail "semantic bytecode changed outside FramePresenter: $cls"
  fi
}
check_same_class 'org.recompile.rg35xx.RG35XXLauncher' launcher
check_same_class 'org.recompile.rg35xx.RG35XXLauncher$1' anon1
check_same_class 'org.recompile.rg35xx.RG35XXLauncher$2' anon2
check_same_class 'org.recompile.rg35xx.RG35XXLauncher$InputPump' inputpump
echo A6_PERF_J1_UNAFFECTED_LAUNCHER_BYTECODE_GATE=PASS

"$JAVA8/bin/javap" -classpath "$PARENT_JAR" -p -c 'org.recompile.rg35xx.RG35XXLauncher$FramePresenter' > "$BUILD/a6-perf-j1-parent-framepresenter.javap"
...
grep -q 'java/lang/reflect/Method.invoke' "$BUILD/a6-perf-j1-parent-framepresenter.javap" || fail "parent reflection invoke unexpectedly absent"
...
echo A6_PERF_J1_FRAMEPRESENTER_DELTA_GATE=PASS
echo A6_PERF_J1_DIRECT_ACCESS_BYTECODE_GATE=PASS
echo A6_PERF_J1_SCOPE_GATE=PASS
...
[ "$INPUT_SHA" = '69a8aeb3940bf3a562a7ae4bcaea10b50f8a8f2c38ad229a5430930f6d' ] && fail "impossible truncated input hash matched"
...
cp "$BUILD/a6-perf-j1-parent-framepresenter.javap" "$DST/A6-PERF-J1-PARENT-FRAMEPRESENTER-JAVAP.txt"
...
A6_PERF_J1_BINARY_SCOPE=RG35XXLauncher_CLASS_FAMILY_ONLY
A6_PERF_J1_SEMANTIC_DELTA=FramePresenter_ONLY
A6_PERF_J1_UNAFFECTED_LAUNCHER_BYTECODE_GATE=PASS
