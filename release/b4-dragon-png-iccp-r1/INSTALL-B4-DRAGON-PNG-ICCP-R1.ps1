...
$ExpectedOldRuntime='e92ac328772f0ed5aa359435b37a30aa58f5b490714b290ea2e5bb19b91dd8a0'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-PNG-ICCP-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-NETWORK-TRACE-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-PNG-ICCP-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-png-iccp-r1-$stamp"
...
try {
 ...
 @(
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-PNG-ICCP-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-PNG-ICCP-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
