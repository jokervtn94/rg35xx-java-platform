...
$ExpectedOldRuntime='90c4d9455e82ca8df5dce667c0a836356dfc69f8b756b6cafe1ea9ac0b9bc1ee'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-HOTPATH-R2 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "R2 runtime precondition failed actual=$(Sha $current)"}
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-hotpath-r2-$stamp"
...
try {
 ...
 $report=Join-Path $Sd 'RG35XX-B4-HOTPATH-R2-INSTALL-RESULT.txt'
 @(
  'RG35XX B4 HOTPATH R2 AB',
  'RESULT=PASS',
  ...
  'VIDEO_MASK_R2=PRESERVED',
  'BOUNDED_LIFECYCLE_LOGGING=PRESERVED',
 ...
 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-HOTPATH-R2-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
 Write-Host 'INSTALL PASS - B4 HOTPATH R2'
}
catch {
