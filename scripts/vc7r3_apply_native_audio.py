...
# launcher argv: keep all proven headless args and reserve always-valid audio fd property slot.
...
''','audio globals')
# helpers before retro_init
anchor='void retro_init(void)\n'
if s.count(anchor)!=1: raise SystemExit('VC7R3 NATIVE AUDIO FAIL: retro_init anchor')
helpers=r'''/* RG35XX-VC7R3-AUDIO-NATIVE: isolated target media runtime. */
#ifndef RG35XX_SOUNDFONT_PATH
#define RG35XX_SOUNDFONT_PATH "/mnt/mmc/BIOS/freej2me.sf2"
#endif

static void rg35xx_core_media_event(int event_type, uint32_t player_id, uint64_t media_time_us)
{
    if(event_type == RG35XX_MEDIA_EVENT_END_OF_MEDIA || event_type == RG35XX_MEDIA_EVENT_LOOPED)
        rg35xx_media_event_queue_push((uint8_t)event_type, player_id, media_time_us);
}

static void rg35xx_release_soundfont_bytes(void)
{
    if(rg35xx_soundfont_bytes) free(rg35xx_soundfont_bytes);
    rg35xx_soundfont_bytes = NULL; rg35xx_soundfont_size = 0;
}

static int rg35xx_load_soundfont_bytes(void)
{
    FILE *f=fopen(RG35XX_SOUNDFONT_PATH,"rb"); long z; size_t n;
    rg35xx_release_soundfont_bytes();
    if(!f) { log_fn(RETRO_LOG_WARN,"RG35XX VC7R3 SoundFont unavailable; MIDI disabled.\n"); return 0; }
    ...
    n=fread(rg35xx_soundfont_bytes,1,(size_t)z,f); fclose(f);
    if(n!=(size_t)z){rg35xx_release_soundfont_bytes();return 0;} rg35xx_soundfont_size=n; return 1;
}

static void rg35xx_send_media_event(const struct rg35xx_media_event_record *e)
{
    unsigned char q[18]; uint64_t t=e->media_time_us;
    q[0]=14; q[1]=0; q[2]=0; q[3]=0; q[4]=13; q[5]=e->event_type;
    q[6]=(e->player_id>>24)&255; q[7]=(e->player_id>>16)&255; q[8]=(e->player_id>>8)&255; q[9]=e->player_id&255;
    q[10]=(t>>56)&255; q[11]=(t>>48)&255; q[12]=(t>>40)&255; q[13]=(t>>32)&255; q[14]=(t>>24)&255; q[15]=(t>>16)&255; q[16]=(t>>8)&255; q[17]=t&255;
    write_to_pipe(pWrite[1],q,sizeof(q));
}
...
{
    int drained=rg35xx_audio_pipe_drain(&rg35xx_java_audio_pipe); size_t frames;
    if(drained<0 && rg35xx_java_audio_pipe.read_fd>=0) log_fn(RETRO_LOG_WARN,"RG35XX VC7R3 audio pipe drain failed.\n");
    frames=rg35xx_mixer_render(rg35xx_audio_run_buffer,RG35XX_AUDIO_FRAMES_PER_RUN);
    if(AudioBatch && soundEnabled && frames>0) AudioBatch(rg35xx_audio_run_buffer,frames);
    { struct rg35xx_media_event_record e; while(rg35xx_media_event_queue_pop(&e)) rg35xx_send_media_event(&e); }
}

static void rg35xx_native_media_init(void)
{
    rg35xx_media_cache_reset(); rg35xx_media_event_queue_reset(); rg35xx_mixer_init(rg35xx_core_media_event);
    if(rg35xx_load_soundfont_bytes() && !rg35xx_media_runtime_init(rg35xx_soundfont_bytes,rg35xx_soundfont_size))
        log_fn(RETRO_LOG_WARN,"RG35XX VC7R3 MIDI runtime init failed; PCM remains available.\n");
}
static void rg35xx_native_media_shutdown(void)
{
    rg35xx_audio_pipe_close(&rg35xx_java_audio_pipe); rg35xx_mixer_reset(); rg35xx_media_event_queue_reset(); rg35xx_media_cache_reset(); rg35xx_media_runtime_shutdown(); rg35xx_release_soundfont_bytes();
}
...
pre=r'''\t/* VC7R3 dedicated Java->native media FD; stdout remains binary video IPC. */
\trg35xx_audio_pipe_init(&rg35xx_java_audio_pipe);
\tif(rg35xx_audio_pipe_create(&rg35xx_java_audio_pipe))
\t{
\t\tint afd=rg35xx_audio_pipe_child_fd(&rg35xx_java_audio_pipe);
\t\tchar prop[64];
\t\tif(afd>=3)
\t\t{
\t\t\tsnprintf(prop,sizeof(prop),"-Dfreej2me.rg35xx.audio.fd=%d",afd);
\t\t\tfree(params[6]); params[6]=strdup(prop);
\t\t}
\t\telse rg35xx_audio_pipe_close(&rg35xx_java_audio_pipe);
\t}

'''
s=s.replace(needle,pre+needle,1)
# child/parent fork ownership
once('''\tif(pid==0) /* child */
\t{
...
\t\tclose(pRead[1]);''','parent fd')
# shutdown before video deinit so Java media fd cannot survive core teardown
