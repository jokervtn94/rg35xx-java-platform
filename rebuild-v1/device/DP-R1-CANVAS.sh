...
OUT=/mnt/mmc/RG35XX-DP-R1-CANVAS.txt
...
LD_LIBRARY_PATH="$APP${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" "$JAMVM" -Xmx64m -cp "$GLIBJ:$PLATFORM" org.recompile.mobile.M19CanvasLifecycleLauncher "$APP/dp-r1-canvas.jar" >>"$OUT" 2>&1
RC=$?; echo JAMVM_EXIT_CODE=$RC >>"$OUT"
check_protected && echo PROTECTED_HASHES=PASS >>"$OUT" || echo PROTECTED_HASHES=FAIL >>"$OUT"
grep -q 'M1_9F_DEVICE_ACCEPTANCE_MARKER=PASS' "$OUT" && [ "$RC" -eq 0 ] && echo RUNTIME_GATE=PASS >>"$OUT" || echo RUNTIME_GATE=FAIL >>"$OUT"
