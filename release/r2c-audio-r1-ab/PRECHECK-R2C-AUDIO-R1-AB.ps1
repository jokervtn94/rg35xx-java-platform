...
$checks=@(
 ...
 @('R2C BIOS','BIOS\freej2me-lr.jar',$ExpectedR2C),
 @('R2C BIOS plus','BIOS\freej2me_plus-lr.jar',$ExpectedR2C),
 @('R2C Java','CFW\java\share\freej2me\freej2me-lr.jar',$ExpectedR2C),
 @('R2C RA hidden','CFW\retroarch\.retroarch\system\freej2me-lr.jar',$ExpectedR2C),
 @('R2C RA system','CFW\retroarch\system\freej2me-lr.jar',$ExpectedR2C)
)
$ok=$true;$out=@('R2C-AUDIO-R1-AB PRECHECK',"TIME=$((Get-Date).ToString('s'))")
...
$out+=('RESULT='+$(if($ok){'PASS'}else{'FAIL'}));$out+=('READY_FOR_R2C_AUDIO_R1_AB='+$(if($ok){'YES'}else{'NO'}))
$out|Set-Content -LiteralPath (Join-Path $Sd 'R2C-AUDIO-R1-AB-PRECHECK.txt') -Encoding ASCII
