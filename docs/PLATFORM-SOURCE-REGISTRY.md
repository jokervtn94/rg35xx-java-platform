...
- First build toolchain: 011O requires explicit ARM/uClibc GCC/G++ executables and verifies their target triplets. Target flags are `-marm -march=armv5te -mtune=arm926ej-s -mfloat-abi=soft`; target `nm` must report no unresolved `rg35xx_*` symbols in the resulting core.
