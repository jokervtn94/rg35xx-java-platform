...
$ExpectedOldRuntime='e8706495bfaed6a9020b395cc65347c76ca3a3d1bd5a1880aed593379fa9f4ed'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-VIDEO-MASK-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4 runtime precondition failed actual=$(Sha $current)"}
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-video-mask-r1-$stamp"
...
try {
 ...
 foreach($name in @('freej2me-java-error.log','freej2me-vc3-early.log')){
   $p=Join-Path $Sd $name
 ...
 $report=Join-Path $Sd 'RG35XX-B4-VIDEO-MASK-R1-INSTALL-RESULT.txt'
 @(
  'RG35XX B4 VIDEO MASK R1 AB',
  'RESULT=PASS',
  ...
  'PRIMARY_VARIABLE=PLATFORMGRAPHICS_LCD_MASK_GATE_ONLY',
  'HOTPATH_CLEANUP=NOT_INCLUDED',
  'AUDIO_CHANGE=NONE',
  'CORE_CHANGE=NONE',
  'FULL_PLATFORM_STABLE=NO'
 )|Set-Content -LiteralPath $report -Encoding ASCII
 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-VIDEO-MASK-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
 Write-Host 'INSTALL PASS - B4 VIDEO MASK R1'
}
catch {
