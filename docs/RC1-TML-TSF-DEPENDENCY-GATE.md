...
Status: STATIC-AUDIT-PASS for dependency/source policy and provenance. Worker and single implementation translation-unit ownership are present; this is not BUILD-PASS.

Task basis: `RGJ-RC1-010F` in `tasklog/RC1-TASKLOG.md` authorizes RESTORE-BY-REIMPLEMENTATION of the historical native TML/TSF worker while preserving `native/rg35xx_midi_backend.c` as the bounded two-context adapter. `RGJ-RC1-011E` reconciles and locks the exact vendor provenance.
...
The physical vendored `tml.h` and `tsf.h` files are still an assembled-source input gate. The repository now contains a deterministic acquisition script plus an offline integrity verifier; neither script weakens the requirement that release/native builds consume exact pinned files.
...
- `tml.h` — TinyMidiLoader v0.7, zlib license; authoritative Git blob `333287377fa860fa7f3d8fe8096d3cf32bfbb6ea`.
- `tsf.h` — TinySoundFont v0.9, MIT license; authoritative Git blob `a81f25d5ca2e210720d646dec2dbfaeb119acb09`.

The blob identities above were independently reconciled against the GitHub Git tree for the exact pinned commit. Earlier provisional identities recorded in the first verification script were incorrect and are superseded by `RGJ-RC1-011E`; do not restore them.
...
## 3. Acquisition and offline verification contract

`native/vendor_tinysoundfont.sh` is the only project acquisition helper. It:

1. initializes a temporary Git repository;
2. fetches exactly the pinned commit by object ID;
3. verifies `FETCH_HEAD:tml.h` and `FETCH_HEAD:tsf.h` against the authoritative Git blob IDs before writing anything into the vendor directory;
4. writes through temporary sibling files;
5. verifies the written files again with `git hash-object`;
6. moves them into `native/vendor/TinySoundFont/` only after both checks pass;
7. invokes `native/verify_tinysoundfont_vendor.sh` as the final offline gate.

Network access is therefore an acquisition-time concern only. Normal native compilation must not download dependencies and must run the verifier against the already-vendored files.
...
Therefore RC1 must not hard-code a guessed `/mnt/mmc/BIOS/*.sf2` path and must not silently bundle an unrelated SoundFont. The TinySoundFont example SoundFonts are upstream examples, not an authoritative RG35XX platform asset.
...
## 12. Remaining build gates

`RGJ-RC1-010F` cannot become BUILD-PASS until all of these are exact-source resolved:

- exact pinned `tml.h` and `tsf.h` are physically present in the assembled native tree and pass `native/verify_tinysoundfont_vendor.sh`;
