...
$ExpectedOldRuntime='86213649b09ce6fca7c55bd14131384a6f8eac07b9896c8a40d9fa1aa5538675'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-SERVICE-REPAINT-SERIALIZE-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-TIMEBASE-TRACE-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-SERVICE-REPAINT-SERIALIZE-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-service-repaint-serialize-r1-$stamp"
...
try {
 foreach($rel in $targets){
   ...
   New-Item -ItemType Directory -Force -Path (Split-Path -Parent $dst)|Out-Null
   $tmp=$dst+'.b4servicerepaintserialnew'
 ...
 @(
  ...
  'RESOURCE_LOAD_TRACE_DIAGNOSTICS=PRESERVED',
  'TIMEBASE_TRACE_DIAGNOSTICS=PRESERVED',
  'SERVICE_REPAINT_SERIALIZATION_FIX=ENABLED',
  'SERVICE_REPAINT_SERIAL_TRACE=ENABLED',
  'IMAGE_DECODE_TRACE_DIAGNOSTICS=PRESERVED',
  'GAMECANVAS_FLUSH_BEHAVIOR_CHANGE=NONE',
  'RESOURCE_LOAD_BEHAVIOR_CHANGE=NONE',
  'TIMEBASE_BEHAVIOR_CHANGE=NONE',
  'DISPLAY_CANVAS_BEHAVIOR_CHANGE=SERVICEREPAINTS_FALLBACK_SERIALIZATION_ONLY',
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-SERVICE-REPAINT-SERIALIZE-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-SERVICE-REPAINT-SERIALIZE-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
 Write-Host 'INSTALL PASS - B4 DRAGON SERVICE REPAINT SERIALIZE R1'
}
catch {
