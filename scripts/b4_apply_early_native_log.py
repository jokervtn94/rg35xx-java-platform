...
# Helper is intentionally tiny and independent of RetroArch logging.
...
# Earliest libretro entry we control.
once(
    'void retro_init(void)\n{\n\t/* init buffers, structs */',
    'void retro_init(void)\n{\n\trg35xx_b4_log("B4 CORE_INIT pid=%ld", (long)getpid());\n\t/* init buffers, structs */',
    "retro_init entry")

# Runtime path existence checkpoint.
once(
    '\tif (!freej2me_present(freej2mePath))\n\t{',
    '\trg35xx_b4_log("B4 RUNTIME_PATH path=%s", freej2mePath);\n\tif (!freej2me_present(freej2mePath))\n\t{\n\t\trg35xx_b4_log("B4 RUNTIME_MISSING path=%s errno=%d", freej2mePath, errno);',
    "runtime path checkpoint")
...
# Game path exists independently from Java stderr/core log.
once(
    'bool retro_load_game(const struct retro_game_info *info)\n{\n\tint len = 0;',
    'bool retro_load_game(const struct retro_game_info *info)\n{\n\tint len = 0;\n\trg35xx_b4_log("B4 LOAD_GAME_ENTER info=%p path=%s", (void*)info, (info && info->path) ? info->path : "<null>");',
    "retro_load_game entry")

once(
    '#ifdef __linux__\n\trealpath(info->path, romPath);',
    '#ifdef __linux__\n\tif(realpath(info->path, romPath) == NULL)\n\t{\n\t\trg35xx_b4_log("B4 GAME_REALPATH_FAIL path=%s errno=%d", info->path, errno);\n\t\tromPath[0] = \'\\0\';\n\t}\n\telse rg35xx_b4_log("B4 GAME_PATH path=%s", romPath);',
    "game realpath checkpoint")

once(
    '\twrite_to_pipe(pWrite[1], loadevent, 5);\n\twrite_to_pipe(pWrite[1], (unsigned char*) romPath, len);',
    '\trg35xx_b4_log("B4 IPC_LOAD_BEGIN len=%d", len);\n\twrite_to_pipe(pWrite[1], loadevent, 5);\n\twrite_to_pipe(pWrite[1], (unsigned char*) romPath, len);\n\trg35xx_b4_log("B4 IPC_LOAD_SENT len=%d", len);',
    "LOAD IPC checkpoint")

once(
    '\twrite_to_pipe(pWrite[1], startupevent, 5);\n\n\tif(!rg35xx_golden_video_init(',
    '\trg35xx_b4_log("B4 IPC_RUN_BEGIN");\n\twrite_to_pipe(pWrite[1], startupevent, 5);\n\trg35xx_b4_log("B4 IPC_RUN_SENT");\n\n\tif(!rg35xx_golden_video_init(',
    "RUN IPC checkpoint")
...
# Fail-closed policy: only observability markers may be added by this stage.
required = (
    'RG35XX_B4_EARLY_LOG', 'B4 CORE_INIT', 'B4 RUNTIME_PATH',
    'B4 JAVA_OPEN_BEGIN', 'B4 LOAD_GAME_ENTER', 'B4 GAME_PATH',
    'B4 IPC_LOAD_SENT', 'B4 IPC_RUN_SENT', 'B4 PIPE_CREATE',
    'B4 FORK_RESULT', 'B4 CHILD_PRE_EXEC', 'B4 CHILD_EXEC_FAIL',
    'B4 PARENT_CHILD_STARTED', 'B4 JAVA_READY', 'B4 CORE_DEINIT'
)
