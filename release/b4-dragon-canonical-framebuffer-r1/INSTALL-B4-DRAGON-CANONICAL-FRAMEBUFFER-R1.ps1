...
$ExpectedOldRuntime='f4b88b2ee0787a74949732a0d5a754301ba49c707de726fab428930f98d33e92'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-CANONICAL-FRAMEBUFFER-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-PNG-ICCP-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-CANONICAL-FRAMEBUFFER-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-canonical-framebuffer-r1-$stamp"
...
try {
 ...
 @(
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-CANONICAL-FRAMEBUFFER-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-CANONICAL-FRAMEBUFFER-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
 Write-Host 'INSTALL PASS - B4 DRAGON CANONICAL FRAMEBUFFER R1'
}
catch {
