...
$ExpectedOldRuntime='ac027e8ac5f5cf0360aa509d966d7f7c40323a595b34fea85003d2c80d0c7c10'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-TIMEBASE-TRACE-R1 INSTALL FAIL: $m"}
...
if((Sha $current) -ne $ExpectedOldRuntime){Fail "B4-DRAGON-RESOURCE-LOAD-TRACE-R1 runtime precondition failed actual=$(Sha $current)"}
...
$rmsReport=Join-Path $Sd 'RG35XX-B4-DRAGON-TIMEBASE-TRACE-R1-RMS-CURRENT.txt'
@(
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-timebase-trace-r1-$stamp"
...
try {
 ...
 @(
  ...
  'TIMEBASE_TRACE_DIAGNOSTICS=ENABLED',
  ...
  'CORE_CHANGE=NONE',
  'STABLE=NO'
 ) | Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-TIMEBASE-TRACE-R1-INSTALL-RESULT.txt') -Encoding ASCII

 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-TIMEBASE-TRACE-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
 Write-Host 'INSTALL PASS - B4 DRAGON TIMEBASE TRACE R1'
}
catch {
