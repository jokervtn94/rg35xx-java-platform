...
# The pinned source wraps case 13 in braces. Anchor only the unique runJar call
# instead of depending on brace/indent layout.
run_anchor = "\t\t\t\t\t\t\t\tMobile.getPlatform().runJar();\n"
if s.count(run_anchor) != 1:
    raise SystemExit("VC7R2 VIEW FAIL: runJar call count=%d" % s.count(run_anchor))
...
helpers = '''\tprivate void rg35xxCaptureFilenameLogicalSize(String path)
\t{
\t\tif(path == null) return;
