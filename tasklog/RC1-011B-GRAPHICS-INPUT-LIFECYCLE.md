...
Status: IMPLEMENTED pending final static audit in `docs/RC1-GRAPHICS-INPUT-LIFECYCLE-AUDIT.md`.
...
5. MODIFY lifecycle load semantics: `beforeGameLoad()` must complete old-game flush/reset before `MobilePlatform.load()` replaces loader/suite context, but it must not mark the new game active before load succeeds. `afterGameLoad()` becomes the explicit success commit point.
6. REPLACE the current pinned Libretro case-15 raw held-key repeat loop with the registered `RG35XXInputEngine.update()` cadence. DOWN/UP are routed through the same reusable sink; `MobilePlatform.pressedKeys` is a mirror only. Slot 20 remains upstream fast-forward state and is not passed into the 0..19 RG35XX input engine.
