...
REPORT=/mnt/mmc/RG35XX-MIYOO-M1.3B.1-DEVICE-RESULT.txt
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" 2>/dev/null && pwd || dirname -- "$0")
PROBE="$SCRIPT_DIR/m1_3b1_exact_keymap_probe"

exec >"$REPORT" 2>&1

echo "RG35XX MIYOO M1.3B.1 DEVICE TEST"
date
echo "SCOPE=RAW_JS0_EXACT_KEYMAP_BASELINE_FIX_ONLY"
...
echo "DO_NOT_PRESS_DURING_FIRST_2_SECONDS=RECOMMENDED"
echo

echo "== RUN EXACT KEYMAP BASELINE-FIX PROBE =="
if [ ! -x "$PROBE" ]; then
...
if [ "$RC" -eq 0 ]; then
  echo "DEVICE_TEST=PASS_EXACT_RAW_KEYMAP_BASELINE_FIX"
else
  echo "DEVICE_TEST=FAIL_EXACT_RAW_KEYMAP_BASELINE_FIX"
fi
echo "DEVICE_PASS=NO"
echo "STABLE=NO"
echo "REPORT=$REPORT"
