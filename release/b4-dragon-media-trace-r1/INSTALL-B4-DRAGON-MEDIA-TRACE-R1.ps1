...
$ExpectedOldRuntime='4f1f126c2e02b4fbc3b8985d3afd0cbb239d35e0f97512a4eb85f25fcbacbc9c'
$NewRuntime='__NEW_RUNTIME_SHA256__'

function Fail([string]$m){throw "B4-DRAGON-MEDIA-TRACE-R1 INSTALL FAIL: $m"}
...
if(!(Test-Path -LiteralPath $q -PathType Leaf)){Fail 'RMS R2 quarantine result missing; keep the two corrupt Dragon Mania stores quarantined for this trace'}
if(-not ((Get-Content -LiteralPath $q -Raw) -match 'RESULT=PASS')){Fail 'RMS R2 quarantine result is not PASS'}
...
$backup=Join-Path $Sd "RG35XX-JAVA-BACKUP\b4-dragon-media-trace-r1-$stamp"
...
try {
 ...
 Set-Content -LiteralPath (Join-Path $Sd 'RG35XX-B4-DRAGON-MEDIA-TRACE-R1-CURRENT-BACKUP.txt') -Value $backup -Encoding ASCII
