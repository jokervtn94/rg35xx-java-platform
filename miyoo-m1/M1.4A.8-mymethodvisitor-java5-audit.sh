...
# M1.4A.8: exact pinned source has opening brace on the same line: switch (owner) {
...
# Semantic sentinels: fail closed if the active rewrite behavior disappears.
for marker in ['org/recompile/mobile/Mobile','"sleep"','javax/microedition/shell/custom/Timer','javax/microedition/shell/custom/TimerTask','javax/microedition/shell/MidletRuntime','mv.visitMethodInsn(opcode, owner, name, desc, itf);']:
    if marker not in t: raise SystemExit('MYMETHODVISITOR_SEMANTIC_MARKER_MISSING:'+marker)
p.write_text(t)
'''
