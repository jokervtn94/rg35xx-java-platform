...
function Fail([string]$Message) { throw "DRAWRGB-AB INSTALL FAIL: $Message" }
...
if ((Get-Sha256 $RuntimeSrc) -ne $expectedRuntime) { Fail 'runtime payload SHA mismatch' }
Write-Host '[3/6] drawRGB A/B runtime payload: VERIFIED'
...
$backupRoot = Join-Path $Sd "RG35XX-JAVA-BACKUP\from-zero-drawrgb-ab-$stamp"
...
try {
    ...
    $result = Join-Path $Sd 'RG35XX-FROM-ZERO-DRAWRGB-AB-INSTALL-RESULT.txt'
    @(
        'RG35XX FROM-ZERO DRAWRGB A/B INSTALL RESULT',
    ...
    Write-Host '[6/6] INSTALL PASS - test Real Football first, then a normal 240x320 title, then KDTT.'
